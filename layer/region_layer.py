...
class RegionLayer(object):
        ...
        for i in range(self.nb_rows):
            ...
            for j in range(self.nb_cols):
                ...
                region = tf.strided_slice(layer, 
                                          begin=[0, top, left, 0], 
                                          end=[10000, top+region_height, left+region_width, 1000],
                                          strides=[1,1,1,1])
    ...
    def _default_func(self, region):
        ...
        with tf.name_scope('region_layer'):
            shortcut=region
            x = tf.layers.batch_normalization(region,axis=-1,training=self.training)
            x = tf.nn.relu(x)
            initial = tf.truncated_normal([3, 3, nb_channels, nb_channels], stddev=0.01)
...
if __name__=='__main__':
    # test
    with tf.Session():
        i=tf.constant(shape=(1,8,8,1),value=np.array(range(64)).reshape((1,8,8,1)))
