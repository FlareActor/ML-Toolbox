...
def remove_layer_from_pb():
    ...
    for idx, node in enumerate(sess.graph_def.node):
    ...
    new_node.CopyFrom(sess.graph_def.node[end_idx])
    ...
    new_input[new_input.index(drop_layer_name)] = sess.graph_def.node[start_idx].name
    drop_layer_name = drop_layer_name.split('/')[0]
    del new_node.input[:]
    new_node.input.extend(new_input)
    print('New input of the end node:\n', new_node.input)

    new_nodes = sess.graph_def.node[:].copy()
